...
@section('content')
<a href="{{ route('time.employee.shift.index') }}" class="btn btn-danger ms-2 px-5"> <i class="fas fa-arrow-left me-2"></i>Back</a>
...
<div class="container mt-4 mx-auto">
    ...
    <div class="card shadow-lg">
        <div class="card-body">
            <form action="{{ route('time.employee.shift.store') }}" method="POST" enctype="multipart/form-data">
                @csrf

                <div class="row mb-3">
                    <div class="col-md-6">
                        <label for="rule_id" class="form-label">Rule Shift</label>
                        <select name="rule_id" class="form-control" required>
                            <option value="" disabled selected>Select a Rule Shift</option>
                ...
                <input type="hidden" name="shift_employees" id="shiftEmployeesInput">
...
<script>
    $(document).ready(function() {
        ...
        $('#removeAllEmployees').on('click', function() {
            ...
            Swal.fire({
                title: 'Are you sure?',
                text: 'You are about to remove all shift employees. This action cannot be undone!',
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    shiftEmployees = [];
                    $('#shiftEmployees').empty();
                    updateHiddenInput();
                    Swal.fire(
                        'Removed!',
                        'All shift employees have been removed.',
                        'success'
                    );
                }
            });
        ...
        function addEmployeesToTable(employeeIds) {
            employeeIds.forEach(id => {
                if (!shiftEmployees.includes(id)) {
                    let emp = employeeList.find(e => e.id == id);
                    if (emp) {
                        shiftEmployees.push(id);
                        $('#shiftEmployees').append(
                            `<tr data-id="${id}">
        ...
        // Remove employee from shift list
        $(document).on('click', '.removeEmployee', function() {
            let id = $(this).data('id');
            shiftEmployees = shiftEmployees.filter(empId => empId != id);
            $(`tr[data-id="${id}"]`).remove();
            updateHiddenInput();
        });

        // Update hidden input with the list of shift employees
        function updateHiddenInput() {
            $('#shiftEmployeesInput').val(shiftEmployees.join(','));
        }
    });
</script>
